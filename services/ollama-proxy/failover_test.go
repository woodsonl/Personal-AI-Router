...
func TestHandleHTTP_404FailoverInferenceOnly(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	newProxy().handleHTTP(rec, httptest.NewRequest(http.MethodPost, "/api/chat", strings.NewReader(`{"model":"llama"}`)))
	if rec.Code != http.StatusOK {
	...
	rec = httptest.NewRecorder()
	newProxy().handleHTTP(rec, httptest.NewRequest(http.MethodGet, "/api/version", nil))
...
func TestHandleHTTP_AggregatesModelList(t *testing.T) {
	...
	events := &recRW{}
	p := NewProxy(NewCodec(events), disc, 11434)
	p.SetSelected("a")
	rec := httptest.NewRecorder()
...
func TestHandleHTTP_AggregatesOpenAIModelList(t *testing.T) {
	a := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = io.WriteString(w, `{"object":"list","data":[{"id":"a","owned_by":"a"},{"id":"shared","owned_by":"first"}]}`)
	}))
	defer a.Close()
	b := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = io.WriteString(w, `{"object":"list","data":[{"id":"shared","owned_by":"second"},{"id":"c","owned_by":"b"}]}`)
	}))
	defer b.Close()
...
func TestHandleHTTP_ModelListEmptyAndUnavailable(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	testProxy(disc, 11434).handleHTTP(rec, httptest.NewRequest(http.MethodGet, "/api/tags", nil))
...
// selection and priority. Ollama's implicit :latest alias is normalized, while
// unknown and known-missing nodes are excluded from inference but remain
// available for non-inference routes.
func TestHandleHTTP_StrictModelRouting(t *testing.T) {
	...
	matchNode.Models = []string{"llama:latest"}
	...
	rec = httptest.NewRecorder()
	p.handleHTTP(rec, httptest.NewRequest(http.MethodGet, "/api/version", nil))
	if rec.Code != http.StatusOK {
